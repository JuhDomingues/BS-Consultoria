...
async function findProperty() {
  ...
  belaVista.forEach(p => {
    console.log('\n---');
    console.log(`ID: ${p.id}`);
    console.log(`Title: ${p.Title || p['Título']}`);
    console.log(`Location: ${p.Location || p['Localização']}`);
    console.log(`Price: ${p.Price || p['Preço']}`);
    console.log(`Category: ${p.Category || p['Categoria']}`);
    console.log(`Bedrooms: ${p.Bedrooms || p['Quartos']}`);
    console.log(`Bathrooms: ${p.Bathrooms || p['Banheiros']}`);
    console.log(`Area: ${p.Area || p['Área']}`);
  });
}
